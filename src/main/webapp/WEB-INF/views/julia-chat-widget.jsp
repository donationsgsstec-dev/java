...
<style>
    ...
    @keyframes juliaGlowPulse {
        0%   { box-shadow: 0 0 20px rgba(255,0,150,.6), 0 0 40px rgba(0,150,255,.4), 0 0 60px rgba(255,0,150,.3); }
        25%  { box-shadow: 0 0 30px rgba(0,255,150,.8), 0 0 60px rgba(255,150,0,.6), 0 0 90px rgba(0,255,150,.4); }
        50%  { box-shadow: 0 0 30px rgba(150,0,255,.8), 0 0 60px rgba(255,0,150,.6), 0 0 90px rgba(150,0,255,.4); }
        75%  { box-shadow: 0 0 30px rgba(255,200,0,.8), 0 0 60px rgba(0,150,255,.6), 0 0 90px rgba(255,200,0,.4); }
        100% { box-shadow: 0 0 20px rgba(255,0,150,.6), 0 0 40px rgba(0,150,255,.4), 0 0 60px rgba(255,0,150,.3); }
    }
    ...
    .julia-chat-messages::-webkit-scrollbar { width: 6px; }
    .julia-chat-messages::-webkit-scrollbar-track  { background: rgba(102,126,234,0.1); border-radius: 10px; }
    .julia-chat-messages::-webkit-scrollbar-thumb  { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 10px; }
    ...
    .julia-chat-input { display: flex; gap: 10px; align-items: center; }
    ...
    .julia-chat-input button:hover { transform: translateY(-2px); box-shadow: 0 6px 20px rgba(102,126,234,0.5); }
    .julia-chat-input button:active { transform: translateY(0); }
    ...
    .julia-chat-toggle img { width: 100%; height: 100%; object-fit: cover; }

    .julia-chat-toggle:hover {
        transform: scale(1.15) rotate(5deg);
        box-shadow: 0 12px 35px rgba(255,0,150,0.6), 0 0 60px rgba(0,255,150,0.5), 0 0 80px rgba(150,0,255,0.4);
    ...
    .julia-chat-tooltip.show { opacity: 1; transform: translateY(0) scale(1); }

    .julia-typing-indicator { display: inline-flex; gap: 4px; padding: 8px 12px; }
...
<button class="julia-chat-toggle" id="juliaChatToggle"
        onclick="toggleJuliaChat()"
        onmouseenter="showJuliaTooltip()"
        onmouseleave="hideJuliaTooltip()"
        title="Chat with Julia">
    <img src="${pageContext.request.contextPath}/images/julia-avatar.svg" alt="Chat with Julia">
</button>
...
<div class="julia-chat-widget" id="juliaChatWidget">
    <div class="julia-chat-header">
        <h5>
            <img src="${pageContext.request.contextPath}/images/julia-avatar.svg"
                 alt="Julia"
...
<script>
    // Avatar path - set once, used everywhere
    var JULIA_AVATAR = '${pageContext.request.contextPath}/images/julia-avatar.svg';

    // Conversation history
    var juliaConversationHistory = [
        { role: 'system', content: 'You are Julia, a helpful AI assistant for Pahappa Limited. Answer questions about the company, its services, and help users with their queries. Be friendly, professional, and concise.' }
    ];

    function showJuliaTooltip() {
        var tooltip = document.getElementById('juliaChatTooltip');
    ...
    function toggleJuliaChat() {
        var chatWidget = document.getElementById('juliaChatWidget');
        var chatToggle = document.getElementById('juliaChatToggle');
        var tooltip    = document.getElementById('juliaChatTooltip');
        ...
        if (chatWidget.classList.contains('active')) {
            if (tooltip) tooltip.classList.remove('show');
            setTimeout(function() {
                var input = document.getElementById('juliaChatInput');
    ...
    function handleJuliaKeyPress(event) {
        if (event.key === 'Enter') sendJuliaMessage();
    }
    ...
    async function sendJuliaMessage() {
        ...
        typingDiv.innerHTML =
            '<div style="display:flex;align-items:flex-start;gap:8px;">' +
                '<img src="' + JULIA_AVATAR + '" alt="Julia" style="width:28px;height:28px;border-radius:50%;border:2px solid #667eea;flex-shrink:0;">' +
                '<div style="display:flex;flex-direction:column;gap:4px;">' +
                    '<div style="font-weight:600;color:#667eea;font-size:12px;">Julia</div>' +
                    '<div class="julia-typing-indicator"><span></span><span></span><span></span></div>' +
                '</div>' +
            '</div>';
        document.getElementById('juliaChatMessages').appendChild(typingDiv);
        scrollJuliaToBottom();

        juliaConversationHistory.push({ role: 'user', content: message });

        try {
            var response = await fetch('${pageContext.request.contextPath}/julia/chat', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ messages: juliaConversationHistory })
            });
            ...
            var indicator = document.getElementById('julia-typing-indicator');
            if (indicator) indicator.remove();

            if (data.reply) {
                addJuliaMessageToChat(data.reply, 'bot');
                juliaConversationHistory.push({ role: 'assistant', content: data.reply });
            } else if (data.error) {
                addJuliaMessageToChat(data.error, 'bot');
            } else {
                addJuliaMessageToChat('No response received. Please try again.', 'bot');
            }
        } catch (err) {
            var indicator = document.getElementById('julia-typing-indicator');
            if (indicator) indicator.remove();
            addJuliaMessageToChat('Connection error: ' + err.message, 'bot');
        }
    }

    function addJuliaMessageToChat(message, sender) {
        var messagesDiv = document.getElementById('juliaChatMessages');
